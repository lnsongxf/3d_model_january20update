...
//zeta_m,   0.7826   ,0,1,BETA_PDF,0.5,0.2;//standard prior for stickiness terms-->check whether another paper attempts to estimate this
//zeta_F,0.6636,0,1,BETA_PDF,0.5,0.2;//standard prior for stickiness terms-->check whether another paper attempts to estimate this
...
//optim=('TolFun',1e-4),
optim=('TolFun',1e-4,'TolX',1e-4,'Display','iter','MaxIter',500,'Hessian','bfgs'),//,'UseParallel',1
//cova_compute=0,
...
shock_groups;
'Net Worth Shocks'=epsiWb,epsiWe;
'Depreciation (Supply) Shocks'=epsiHd,epsiHk;
'Productivity'=epsiA;
'Preference Shocks'=epsiJ,epsiEC;
'Uncertainty Shocks'=epsiSe,epsiSm,epsiSB;
'Mark-up Shocks'=epsimarkup_m, epsimarkup_F;
'cost-push'=epsiH,epsiK;
'Bank Capital Shock'=epsiECAB;
//'labor supply'=epsiEL;
end;
...
//for jj=1:10
//        disp(jj)
//      stoch_simul(order=1,irf=40,nograph,nocorr,nomoments,nodecomposition,nodisplay,noprint,relative_irf);
//        load xx.mat;
...
//dy_data, dq_H_data,int_rate_HH_data,int_rate_business_data,bank_rate_data,dbe_data,dbm_data,dw_data,dc_data,dinve_data;// , dq_H_data,d_b_to_Y_data,int_rate_HH_data,bank_rate_data,bsp_H_data ,d_b_to_Y_data; 

   stoch_simul(order=1,irf=60,nograph,nocorr,nomoments,nodecomposition,nodisplay);

//calib_smoother(datafile='estimation_dataset_quarterly.mat')dy_data, Y_net;
//
//shock_decomposition(parameter_set=posterior_mode,datafile='estimation_dataset_quarterly.mat')dy_data, dq_H_data,dc_data,dinve_data;
