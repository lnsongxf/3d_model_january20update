...
set_Cyphi_H =data.parameters.Cyphi_H_counterfactual; %parameter on capital requirement on mortgage banks %18 ==>pro-cyclical component.
set_Cyphi_F=data.parameters.Cyphi_F_counterfactual; % parameter on capital requirement on corporate banks %20
set_phi_Hs=data.parameters.phi_Hs_counterfactual; % capital adequecy ratio? same equation as above, equation shut off?  %19 --> non-pro cyclical component
set_phis=data.parameters.phis_counterfactual; % is this the capital adequecy ratio?  this is equated to phi, phi is not used anywhere??  is it the same as phi_F and phi_H maybe?  %69
set_phi_Fs =data.parameters.phi_Fs_counterfactual; % capital adequecy ratio? REDUNDANT? appears in the same equation as above, the equation is shut off.  %21
%LTV rule
set_epsilonH1s=data.parameters.epsilonH1s_counterfactual; %***LTV LIMIT?  %61
set_epsilonF1s=data.parameters.epsilonF1s_counterfactual; %*** LTV LIMIT?  =epsilonH  & =epsilonF respectively. 
set_LTVHrule=data.parameters.LTVHrule_counterfactual;%***
set_LTVFrule=data.parameters.LTVFrule_counterfactual;%***
...
% save('LTV1_parameter_values.mat',tosave);
save LTV1_parameter_values.mat  -regexp ^(?!(variableToExclude1|variableToExclude2)$).
% save LTV1_parameter_values.mat -except ans 
