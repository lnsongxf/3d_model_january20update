...
// 24 Budget Constraint
  C_s - C_e - C_b + q_H*(H_s-(1-deltaH)*H_s(-1))+ D = w*L_s +R_DD*D(-1) - Tr*a_s + PI + PH ; //+ chi_e*W_e*(1-EWe) + chi_b*(W_b)*(1-EWb);
...
W_e= (1-Gamma_e)*R_K*q_K(-1)*K(-1); // *EWe
...
'Bank Risk'=epsiSB;
'Business Dividend Shock'=epsiWe;
'Bank Dividend Shock'=epsiWb;
