...
% load('c:\users\tolga\desktop\3d_model_tolga\version_oct_15\LTV1\metropolis\LTV1_mh1_blck2.mat');
% logpost_final(N+1:2*N)=logpo2(N+1:end);
% posteriorDist(N+1:2*N,:)=x2(N+1:end,:);
% 
% load('c:\users\tolga\desktop\3d_model_tolga\version_oct_15\LTV1\metropolis\LTV1_mh1_blck3.mat');
% logpost_final(2*N+1:3*N)=logpo2(N+1:end);
% posteriorDist(2*N+1:3*N,:)=x2(N+1:end,:);
% 
% load('c:\users\tolga\desktop\3d_model_tolga\version_oct_15\LTV1\metropolis\LTV1_mh1_blck4.mat');
...
for paraInd=1:length(posteriorMode)
...
subplot(4,4,index);hist(plot_var);
hold on;
