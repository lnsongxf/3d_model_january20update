...
dy_data=Y_obs-Y_obs(-1)+gamma_y;
dq_H_data=q_H_obs-q_H_obs(-1)+gamma_dq_H;
dbe_data=b_e_obs-b_e_obs(-1)+gamma_dbe;
//dbe_data=100*log(b_e/b_e(-1));
dbm_data=b_m_obs-b_m_obs(-1)+gamma_dbm;
...
psi_i,   1.5593,1,100,NORMAL_PDF,4,1.5;//standard prior from SW
psi_h,  17.6743 ,1,100,NORMAL_PDF,4,1.5;//standard prior from SW
//zeta_m, 0.6383  ,0,1,BETA_PDF,0.5,0.2;//standard prior for stickiness terms-->check whether another paper attempts to estimate this
//zeta_F,0.7820,0,1,BETA_PDF,0.5,0.2;//standard prior for stickiness terms-->check whether another paper attempts to estimate this

stderr epsiHd,0.1,0,99,          UNIFORM_PDF,10, 5.773502691896258;//  GAMMA_PDF,4,4;//
stderr epsiHk,0.01,0,99,    UNIFORM_PDF,10, 5.773502691896258;//  GAMMA_PDF,4,4;//
...
//optim=('TolFun',1e-4),
optim=('TolFun',1e-4,'TolX',1e-4,'Display','iter','MaxIter',200,'Hessian','bfgs'),//,'UseParallel',1
cova_compute=0,
...
shock_groups;
'net worth'=epsiWb,epsiWe;
'supply'=epsiHd,epsiHk;
'productivity'=epsiA;
'preference'=epsiJ,epsiEC;
'risk shocks'=epsiSe,epsiSm,epsiSB;
'mark-up'=epsimarkup_m, epsimarkup_F;
'cost-push'=epsiH,epsiK;
...
//for jj=1:10
//        disp(jj)
   //     stoch_simul(order=1,irf=40,nograph,nocorr,nomoments,nodecomposition,nodisplay,noprint,tex,periods=5000,drop=2500,relative_irf);
//        load xx.mat;
//        xx(:,:,jj)=oo_.endo_simul;
//        save xx.mat xx;
//end
 //stoch_simul(order=1,irf=40,nograph,nocorr,nomoments,nodecomposition,nodisplay,noprint,tex,relative_irf);

stoch_simul(order=1,irf=40,periods=10000,drop=5000,tex,relative_irf);
//shock_decomposition(datafile='estimation_dataset_quarterly.mat',parameter_set=posterior_mode,init_state=1,use_shock_groups,colormap=parula)
//dy_data, dq_H_data,int_rate_HH_data,int_rate_business_data,bank_rate_data,dbe_data,dbm_data,phib,dc_data;// , dq_H_data,d_b_to_Y_data,int_rate_HH_data,bank_rate_data,bsp_H_data ,d_b_to_Y_data; 
...
//shock_decomposition(datafile='estimation_dataset_quarterly.mat',parameter_set=posterior_mode,init_state=1,use_shock_groups,colormap=parula)dy_data, dq_H_data,d_b_to_Y_data,int_rate_HH_data,int_rate_business_data,bank_rate_data,bsp_H_data,dbe_data,dbm_data,dw_data,dc_data,dinve_data,CR_data;// , dq_H_data,d_b_to_Y_data,int_rate_HH_data,bank_rate_data,bsp_H_data ,d_b_to_Y_data; 
//realtime_shock_decomposition(datafile='estimation_dataset_quarterly.mat',parameter_set=posterior_mode)dy_data;//dy_data, dq_H_data,d_b_to_Y_data,int_rate_HH_data,bank_rate_data,bsp_H_data,dbe_data,dbm_data,dw_data,dc_data,dinve_data;// , dq_H_data,d_b_to_Y_data,int_rate_HH_data,bank_rate_data,bsp_H_data ,d_b_to_Y_data; 
//shock_decomposition(datafile='estimation_dataset_quarterly.mat',parmeter_set=posterior_mode)dy_data,dw_data,dc_data,dinve_data,bsp_H_data;// , dq_H_data,d_b_to_Y_data,int_rate_HH_data,bank_rate_data,bsp_H_data ,d_b_to_Y_data; 
//stoch_simul(order=1,irf=40,nograph,pruning,relative_irf);//,Gamma_m,Gamma_e,Gamma_B,Gamma_m_1,Gamma_e_1,Gamma_B_1,Gamma_B_1,G_m,G_e,G_B,G_B,G_m_1,G_e_1,F_pi,F_pe,F_pB,F_pB;
//Y_net_obs,dy_data,dw_data,dinve_data,dq_H_data,bank_rate_data,bsp_H_data,b_m_obs;
//stoch_simul(order=1,irf=40,nograph,relative_irf);//def_rate_e,def_rate_m,def_rate_B,def_rate_B,F_pi,F_pe,F_pB,F_pB,D;//Y_net_obs,dy_data,dw_data,dinve_data,dq_H_data,bank_rate_data,bsp_H_data,b_m_obs;
//R_D,R_D_obs,dy_data, dq_H_data,dbe_data,dbm_data,bank_rate_data,def_rate_e,def_rate_m,def_rate_B,def_rate_B,b_tot,b_tot_obs,Y_net,Y_net_obs,credit_gap_obs,w_obs,L_obs,b_m_obs,Tr_obs,w,L,b_m,Tr;//,Y_net,Y_net_obs b_tot b_tot_obs,b_m,b_m_obs,b_e,b_e_obs,dbe_data,dbm_data,bsp_H_data,bsp_F_data;
